        ...
        public ActionResult UpdateFav(FavDTO model)
        {
            ...
            else
            {
                ...
                if (model.LogoImage != null)
                {
                    ...
                    Bitmap LogoImaze = new Bitmap(postedfilelogo.InputStream);
                    Bitmap resizelogoImage = new Bitmap(LogoImaze, 100, 100);
                    string ext = Path.GetExtension(postedfilelogo.FileName);//yahan se
                    if (ext == ".ico" || ext == ".jpg" || ext == ".jpeg" || ext == ".png")
                    {
                        string favunique = Guid.NewGuid().ToString();
